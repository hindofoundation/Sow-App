      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES.map((service) => (
            <div 
              key={service.id} 
              className="group bg-zinc-900 p-10 border border-zinc-800 hover:border-yellow-400 transition-all duration-300 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <span className="text-6xl font-black text-white">0{service.id}</span>
              </div>
              ...
              <h3 className="text-xl font-black text-white mb-4 uppercase tracking-wide group-hover:text-yellow-400 transition-colors">
                {service.title}
              </h3>
